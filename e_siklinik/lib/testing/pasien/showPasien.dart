  ...
  Future<void> _getPasienDetail() async {
    try {
      final response = await http.get(
          Uri.parse("http://192.168.0.5:8080/api/pasien/show/${widget.pasienId}"));
  ...
  Future<void> _getRiwayatCheckup() async {
    try {
      final response = await http.get(Uri.parse(
          "http://192.168.0.5:8080/api/riwayat-pasien/${widget.pasienId}"));
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
              ...
              children: [
                CircleAvatar(
                  backgroundImage: NetworkImage(
                      'http://192.168.0.5:8080/storage/' + pasienDetail!['image']),
                ),
