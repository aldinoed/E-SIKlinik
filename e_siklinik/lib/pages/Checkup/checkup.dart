...
class _AddCheckupState extends State<AddCheckup> {
  ...
  final String apiPostCheckupResult =
      "http://192.168.0.5:8080/api/checkup-obat/insert";
  final String apiGetAllObat = "http://192.168.0.5:8080/api/obat";
  ...
  Future<void> _getAssesmentDetail() async {
    try {
      final response = await http.get(
        Uri.parse(
            "http://192.168.0.5:8080/api/checkup-assesmen/show/${widget.assesmentId}"),
      );
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
