...
class _RiwayatCheckupState extends State<RiwayatCheckup> {
  ...
  Future<void> _getCheckupDetail() async {
    try {
      final response = await http.get(
        Uri.parse(
            "http://192.168.0.107:8000/api/checkup-result/show/${widget.checkupId}"),
      );
      ...
      builder: (BuildContext context) {
        return Dialog(
            child: checkupDetail!['url_file'] != null
                ? Image.network('http://192.168.0.107:8000/storage/' +
  ...
  Future<void> _generatePdf() async {
    ...
    if (checkupDetail!['url_file'] != null) {
      final imageUrl =
          'http://192.168.0.107:8000/storage/${checkupDetail!['url_file']}';
